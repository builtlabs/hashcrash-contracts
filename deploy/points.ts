...
import { SEASON_THREE, DIRTY } from "./helpers/season-data";
import fs from "fs";
...
export default async function (runtime: HardhatRuntimeEnvironment) {
    ...
    const totalGrind = ethers.parseEther("111111111");

    const seasonId = 3n;

    await tx(grind.approve(pool.target, totalGrind));
    await tx(pool.populateGrind(seasonId, totalGrind));
    await tx(pool.populatePoints(seasonId, SEASON_THREE));
    await tx(pool.openClaim(seasonId));

    const totalPoints = SEASON_THREE.reduce((acc, season) => acc + season.points, 0);
