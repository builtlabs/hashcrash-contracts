...
abstract contract LootTable {
    ...
    function getProbabilities() external pure returns (uint256[] memory) {
        return _getProbabilities();
    }

    /// @notice Returns the hash of the loot table length, multipliers and probabilities.
    function getProvenance() external view returns (bytes32) {
        return _PROVENANCE;
    }
    ...
    function _profit(uint256 _value, uint256 _index) internal pure validIndex(_index) returns (uint256) {
        return (_value * (_multiplier(_index) - MULTIPLIER_DENOMINATOR)) / MULTIPLIER_DENOMINATOR;
    }

    function _getDeadIndex(
        bytes32 _salt,
        uint256 _roundStartBlock
    ) internal view returns (uint64 deadIndex, bytes32 proof) {
    ...
    function _getMultipliers() private pure returns (uint256[] memory) {
    ...
    function _getProbabilities() private pure returns (uint256[] memory) {
        uint256 length = _getLength();
    ...
    function _getProofHash(bytes32 _salt, bytes32[] memory _hashes) private view returns (bytes32) {
        return keccak256(abi.encode(_PROVENANCE, _salt, _hashes));
    }
