...
describe("LootTable", function () {
        ...
        it("Should set the provenance", async function () {
            ...
            const encoded = ethers.keccak256(
                ethers.AbiCoder.defaultAbiCoder().encode(
                    ...
                    [length, multipliers, probabilities, 1e6, oneEther]
                )
            );

            expect(await sut.getProvenance()).to.equal(encoded);
        });
    });
    ...
    describe("getProvenance", function () {
        it("Should return the provenance", async function () {
            const { sut } = await loadFixture(fixture);
    ...
    describe("getDeadIndex", function () {
        ...
        it("Should get the expected dead index", async function () {
            ...
            const deathData = await sut.getDeadIndex(salt, startBlock);
        ...
        it("Should get the expected number of hashes", async function () {
            ...
            const provenance = await sut.getProvenance();
            const deathData = await sut.getDeadIndex(salt, startBlock);
    ...
    describe("_profit", function () {
        it("Should revert if the index is out of range", async function () {
            const { sut } = await loadFixture(fixture);

            await expect(sut.profit(oneEther, 1)).to.be.revertedWithCustomError(sut, "InvalidIndex");
        });

        it("Should return the value multiplied by the multiplier - the original of the valid index", async function () {
            const { sut } = await loadFixture(fixture);

            expect(await sut.profit(oneEther, 0)).to.equal(ethers.parseEther("2") - oneEther);
        });
    });
});
